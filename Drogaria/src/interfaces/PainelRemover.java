...
public class PainelRemover extends JPanel implements ActionListener, ListSelectionListener, KeyListener {
	...
	public PainelRemover() {
		...
		btnRemover.setBounds(310, 350, 180, 40);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (arg0.getSource().equals(btnRemover)) {
			...
			modelo.setRowCount(0);
			for (int i = 0; i < Janela.getInstance().listaRemedios.size(); i++) {
				if (valor.equals(Janela.getInstance().listaRemedios.get(i).getNome())) {
					Janela.getInstance().listaRemedios.remove(i);
				}
			}
		}
